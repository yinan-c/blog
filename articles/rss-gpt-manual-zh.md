...
- 关于 OpenAI API 的用量，为了尽可能保证费用尽可能低，本项目采取了以下默认措施：
  ...
  - 根据文本长度选取不同的模型，而如果文本长度超过了 16k，`gpt-4-1106-preview` 模型会被使用，否则默认使用 `GPT-3.5 Turbo`。如果你想要使用不同的模型，可以在 `main.py` 文件修改第 227 - 243 行中传入 `gpt_summary` 函数的参数 `model="your_model"` 。请参考 [OpenAI API Pricing](https://openai.com/pricing/) 了解不同 OpenAI 模型的不同价格。
